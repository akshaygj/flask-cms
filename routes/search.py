...
def search(query):
    ...
    queryContent = cursor.execute(
        f"select * from posts where content like '%{query}%'"
    ).fetchall()
    ...
    queryContent = cursor.execute(
        f"select * from posts where content like '%{queryNoWhiteSpace}%'"
    ).fetchall()
    queryAuthors = cursor.execute(
